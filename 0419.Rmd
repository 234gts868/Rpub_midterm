...
```{r,warning=FALSE}
#>>>>>>> 20e75f48e910e15b9715e0d79dd2891bbcbfcfc1
library(tm)
library(stringr)
...
ORIP$weekday<- weekdays(ORIP$order_purchase_timestamp.x)%>%
  factor(levels= c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday","Sunday"))

ORIP$hour<- hour(ORIP$order_purchase_timestamp.x)

dailyOrder <- as.data.frame(table(ORIP$weekday, ORIP$hour))
