...
RI$review_comment_title = NULL
RI$review_comment_message = NULL
RI$review_creation_date= NULL
RI$review_answer_timestamp = NULL
RI$order_item_id = NULL
RI$product_id = NULL
RI$shipping_limit_date = NULL
RI$price = NULL
RI$freight_value = NULL
```
...
計算商家評論次數
```{r}
SRI= RI %>% group_by(seller_id) %>% 
  summarize(n_distinct(review_id)) %>%
...
```{r}
SRI3 = merge(SRI,SRI2,by="seller_id")
SRI3$seller_zip_code_prefix.y = NULL
SRI3$seller_city.y = NULL
SRI3$seller_state.y = NULL
colnames(SRI3)[2]="reviews_times"
colnames(SRI3)[6] = "average_score"
colnames(SRI3)[3] = "zip_code_prefix"
colnames(SRI3)[4] = "seller_city"
colnames(SRI3)[5]="seller_state"
```
...
GSRI3 = geolocation %>%
  ...
  inner_join(SRI3,by = "zip_code_prefix")
```
...
```{r}
geo_late_customer$order_status = NULL
geo_late_customer$order_purchase_timestamp = NULL
geo_late_customer$order_approved_at = NULL
geo_late_customer$order_delivered_carrier_date = NULL
geo_late_customer$customer_unique_id = NULL
...
```
```{r}
as.numeric(geo_late_customer$late_time)
